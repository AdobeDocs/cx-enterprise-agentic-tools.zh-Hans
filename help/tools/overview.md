...
并非每个代理工具都满足同样的需求。 探索每个应用程序的作用、何时使用以及如何开始 — 以便您选择适合自己情况的正确起点。
...
将MCP服务器视为AI客户端与CX企业应用程序之间的实时线路。 连接一次，您的AI就可以查询营销活动、拉取受众、检查历程状态等 — 所有这些都使用纯语言，无需代码。
...
“代理技能”是Adobe的域专业知识，按照您的代理可遵循的说明进行编码。 与其希望您的代理能够指出正确的步骤，不如用一种技能告诉它该做什么 — 可靠、可重复而且已经针对CX Enterprise工作流进行了调整。
...
这些工具旨在协同工作，将它们组合在一起就是您从Adobe AI中获得最大价值的地方。 代理技能可以指导AI客户端如何使用MCP服务器，使代理在CX Enterprise工作流程中保持正确跟踪。 技能还可以指导如何以及何时调用API，将Adobe最佳实践护栏添加到自定义自动化。 你不必只选一个。
